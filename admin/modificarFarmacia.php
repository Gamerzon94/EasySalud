<?php
	...
	$usuario = "2";
        ...
        $farmacia = "4";
        $id = $_GET['id'];
        $farma = buscarFarmaciaEspecifica($id);
        $auxRepresentante = cargarRepresentantes();
?>
<!--Author: W3layouts
...
<!-- script-for sticky-nav -->
		...
		<!-- /script-for sticky-nav -->
                <?php while($filas = mysqli_fetch_array($farma)){ ?>
<!--inner block start here-->
<div class="inner-block">
    <div class="blank">
    	<h2>Modificar farmacia <?php echo $filas['D_NOMBRE_FARMACIA'];?></h2>
    	<div class="blankpage-main">
    		<form action="modificarFarma.php" method="post" onsubmit="return validar();">
                    <table>
                        <tr>
                            <td>Nombre de la farmacia <br/>
                                <input type="hidden" id="id" name="id" value="<?php echo $filas['C_ID_FARMACIA'];?>">
                                <input type="text" name="nom" placeholder="Nombre de la Farmacia" required="" value="<?php echo $filas['D_NOMBRE_FARMACIA'];?>"></td>
                            <td>Pagina Web de la farmacia <br/><input type="text" name="web" placeholder="Pagina Web Farmacia" required="" value="<?php echo $filas['D_WEB_FARMACIA'];?>"></td>
                            <td>Representante farmacia <br/><select name="rep" id="rep">
			<option value="0">Seleccione representante</option>
			<?php while($representante = mysqli_fetch_array($auxRepresentante)){
                            if($filas['C_ID_USUARIO'] == $representante['C_ID_USUARIO']){?>
                        <option value="<?php echo $representante['C_ID_USUARIO']; ?>" selected><?php echo $representante['S_RUN_USUARIO']." - ".$representante['D_NOMBRE_USUARIO']." ".$representante['D_APELLIDO_USUARIO']; ?></option>
                            <?php }else{ ?>
                                <option value="<?php echo $representante['C_ID_USUARIO']; ?>"><?php echo $representante['S_RUN_USUARIO']." - ".$representante['D_NOMBRE_USUARIO']." ".$representante['D_APELLIDO_USUARIO']; ?></option>
                            <?php } }?>
                                </select></td>
                        </tr>
                        <tr><td><div class="forgot-top-grids">
                            <div class="clearfix"> </div>
                                ...
                                <input type="submit" name="modificar" value="Modificar farmacia"></td></tr>
                    </table>
		</form>
    	</div>
    </div>
