<?php
	...
	$usuario = "2";
        ...
        if(isset($_POST['premium'])){
            if($_POST['premium']=='todas'){
                $resultado = mostrarUsuarios();
                $auxProfesional = obtenerProfesionales();
                $forma = TRUE;
                $opcion = 1;
            }else if($_POST['premium']=='pre'){
                $resultado = obtenerProfesionalesPremiumConNombre();
                $opcion = 2;
            }else if($_POST['premium']=='far'){
                $resultado = obtenerProfesionalesNoPremiumConNombre();
                $opcion = 3;
            }else if($_POST['premium']=='farpre'){
                $resultado = mostrarUsuariosActivos();
                $opcion = 4;
            }else if($_POST['premium']=='nor'){
                $resultado = obtenerProfesionalesNoPremiumConNombre();
                $opcion = 5;
            }else{
...
<div class="inner-block">
    <div class="blank">
    	...
    	<div class="blankpage-main">
            <?php if($forma==TRUE){ 
                $premium = 0;
                $noPremium = 0;
                while ($profesionales = mysqli_fetch_array($auxProfesional)){
                    if($profesionales["L_PREMIUM"]==TRUE){
                        $premium++;
                    }else if($profesionales["L_PREMIUM"]==FALSE) {
                        $noPremium++;
                    }
                }?>
    ...
    <script type="text/javascript">
      ...
      function drawChart() {
        ...
        var data = google.visualization.arrayToDataTable([
          ...
          ['Premium',     <?php echo $premium; ?>],
          ['Normales',     <?php echo $noPremium; ?>]
        ]);
    ...
    <?php $resultado = obtenerProfesionalesConNombre();
                 } ?>
    <table border=\"1\">
			<tr>
				<td>
			...
			while($filas = mysqli_fetch_array($resultado)){ ?>
				<tr>
				<td><a href="detallesUsuario.php?id=<?php echo $filas['C_ID_PROFESIONAL']; ?>"><?php echo $filas['S_RUN_USUARIO'] ?></a></td>
			...
			<?php } ?>
                ...
                <?php if($opcion==1){
                    ?><a href="generar-pdf.php?prof=1">Descargar pdf</a><?php
                }else if($opcion==2){
                    ?><a href="generar-pdf.php?profPremium=1">Descargar pdf</a><?php
                }else if($opcion==3){
                    ?><a href="generar-pdf.php?farmaciasTodas=1">Descargar pdf</a><?php
                }else if($opcion==4){
                    ?><a href="generar-pdf.php?farmaciasActivas=1">Descargar pdf</a><?php
                }else if($opcion==5){
                    ?><a href="generar-pdf.php?noPremium=1">Descargar pdf</a><?php
                }?>
    	</div>
    </div>
</div>
