<?php
	...
	$usuario = "2";
        ...
        $sucursal = buscarSucursalEspecifica($id);
?>
<!--Author: W3layouts
...
<div class="inner-block">
    <div class="cols-grids panel-widget">
        <?php while($filas = mysqli_fetch_array($sucursal)){ 
            $idSucursal = $filas["C_ID_SUCURSAL"];?>
        <h2>Detalles sucursal "<?php echo $filas['D_NOMBRE_SUCURSAL'].", ".$filas['D_NOMBRE_FARMACIA']; ?>"</h2>
        Nombre: <?php echo $filas['D_NOMBRE_SUCURSAL'] ?>.<br/>
        Dirección: <?php echo $filas['D_DIRECCION_SUCURSAL'].", ".$filas['D_NOMBRE_COMUNA'].", ".$filas['D_NOMBRE_REGION'] ?>.<br/>
        Teléfono: <?php echo $filas['N_TELEFONO_SUCURSAL'] ?>.<br/>
        Estado: <?php if($filas['L_ACTIVA'] == TRUE){
                                        echo "Activa";
                                    }else{
                                        echo "Inactiva";
                                    } ?>.<br/>
        ...
        $pagosSucursal = obtenerPagosSucursalEspecifica($idSucursal); ?>
        ...
        <?php $count = 0;
        while($auxPagos = mysqli_fetch_array($pagosSucursal)){ 
            $count++;?>
            <tr>
                <td>
                    F-<?php echo $auxPagos["C_ID_PAGO"] ?>
                </td>
                <td>
                    <?php echo $auxPagos["D_NOMBRE_PLAN"] ?>
                </td>
                <td>
                    $<?php echo $auxPagos["C_VALOR"] ?>.-
                </td>
                <td>
                    <?php $auxFechaPago = obtenerFechaPagosSucursalEspecifica($auxPagos["C_ID_SUCURSAL"]);
                    while($fechaPago = mysqli_fetch_array($auxFechaPago)){
                        echo $fechaPago["F_FECHA_REGISTRO"];
                        break;
                    } ?>
                </td>
                <td>
                    <a href="generar-pdf.php?boleta=<?php echo $auxPagos["C_ID_PAGO"] ?>"><img src="../images/pdf.png" title="Descargar Boleta"></a>
                </td>
            </tr>
        <?php } 
        if($count == 0){
            echo "<tr><td colspan='5'><center>No se registran pagos</center></td></tr>";
        }?>
        </table></center>
    </div>
</div>
