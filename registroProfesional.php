<?php
	...
	$usuario = "2";
        ...
        if(isset($_GET[id])){
            $idUsuario = $_GET['id'];
            $esProfesional = verificarProfesional($idUsuario);
	...
	window.location.href='index.php';
                ...
                if(isset($_POST['idProfesional'])){
                            $buscarProfesional = buscarUsuarioEspecifico($idUsuario);
                            while($auxUsuario = mysqli_fetch_array($buscarProfesional)){
                                $idComuna = $auxUsuario["C_ID_COMUNA"];
                                $direccion = $auxUsuario["D_DIRECCION_USUARIO"];
                            }
                            $equip = crearProfesional($idUsuario,$idComuna,$direccion,$_POST['descri']);
                            $nombre = $_FILES['imagen']['name'];
                            $exten = explode(".", $nombre);
                            $extension = end($exten);
                            $nombrer = strtolower($nombre);
                            $cd=$_FILES['imagen']['tmp_name'];
                            $ruta = "images/prof/" . $equip.".".$extension;
                            $destino = "images/prof/".$equip.".".$extension;
                            $resultado = @move_uploaded_file($_FILES["imagen"]["tmp_name"], $ruta);

                            if (!empty($resultado)){

                                actualizarImagenProfesional($equip, $destino);
                                header('Location: registroProfesional2.php?id='.$equip);
                                            }else{
                                header('Location: registroProfesional2.php?id='.$equip);
                                                }
                    }
  ...
  <main class="hoc container clear"> 
    ...
    <div class="content"> 
      ...
      <h1>Registro</h1>
          ...
          En este paso podrá colocar una breve descripción de su perfil y añadir una imagen.
          <form action="" method="post" enctype="multipart/form-data">
              <table>
                  <tr>
                      ...
                      <td>
                          <textarea name="descri" id="descri" rows="4" cols="50" required></textarea>
                      </td>
                  </tr>
                  <tr>
                      <td>
