<?php
	...
	$rol = $_COOKIE["rol"];
        ...
        if(isset($_GET["id"])){
        $auxPacientes = buscarPacienteEspefico($_GET["id"]);
        }else{
	...
	$usuario = "2";
        ...
        if(isset($_POST['dir'])){
            ...
            $dir=$_POST['dir'];
            $dep=$_POST['dep'];
            modificarPaciente($_GET["id"], $com, $tel, $movil, $dir, $dep);
            header("Location:pacientes.php");
        }
?>
<!DOCTYPE html>
    ...
    <div class="content"> 
        <?php if(isset($_COOKIE['rol'])){
            if($rol != $usuario){
                ...
                En caso de necesitar cambiar los nombres o eliminar el paciente, favor comunicarse con la administración.
                <?php while($auxPacien = mysqli_fetch_array($auxPacientes)){ ?>
                <form action="" method="post">
                    <table>
                        <tr>
                            <td>
            <label for="name">Nombre <span>*</span></label></td><td>
                                <?php echo $auxPacien["D_NOMBRE_PACIENTE"] ?></td></tr><tr>
            <td>
            <label for="name">Apellido <span>*</span></label></td><td>
                <?php echo $auxPacien["D_APELLIDO_PACIENTE"]?></td></tr><tr>
            <td>
                <label for="name">Región <span>*</span></label></td><td>
            <select name="region" id="region">
			<option value="0">Seleccione región</option>
			<?php while($row = mysqli_fetch_array($regiones)){
                            if($row['C_ID_REGION'] == $auxPacien["C_ID_REGION"]){ ?>
            ...
            <input type="text" name="dir" id="dir" value="<?php echo $auxPacien["D_DIRECCION_PACIENTE"]?>" required></td></tr>
            <tr><td>
            <label for="name">Depto.</label></td><td>
            <input type="text" name="dep" id="dep" value="<?php echo $auxPacien["S_DEPTO_PACIENTE"]?>" ></td>
                        </tr><tr><td >
            <label for="name">Genero <span>*</span></label></td>
                <td colspan="2">
                    <?php if($auxPacien["S_SEXO_PACIENTE"] == "M"){ ?>
                    Masculino.
                    <?php }else{ ?>
                       Femenino.
                    <?php } ?>
            ...
            <label for="name">Teléfono</label></td>
                <td colspan="2">
                    <?php if($auxPacien["N_TELEFONO_PACIENTE"] == 0){ ?>
                        <input type="number" name="tel" id="tel" value="">
                    <?php }else{?>
                    <input type="number" name="tel" id="tel" value="<?php echo $auxPacien["N_TELEFONO_PACIENTE"]?>">
                    <?php } ?>
                </td></tr>
                        <tr><td >
            <label for="name">Móvil <span>*</span></label></td>
                <td colspan="2">
                    <?php if($auxPacien["N_MOVIL_PACIENTE"] == 0){ ?>
                        <input type="number" name="cel" id="cel" value="" required></td></tr>
                    <?php }else{?>
                    <input type="number" name="cel" id="cel" value="<?php echo $auxPacien["N_MOVIL_PACIENTE"]?>" required></td></tr>
                    <?php } ?>
                        <tr>
                            <td colspan="2">
