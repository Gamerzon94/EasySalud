<?php
	...
	$usuario = "2";
        ...
        if(isset($_GET[id])){
            $esProfesional = verificarProfesional($_GET['id']);
            $auxId = buscarIdProfesional($_GET['id']);
            while($auxFilas = mysqli_fetch_array($auxId)){
                $idProfesional = $auxFilas['C_ID_PROFESIONAL'];
            }
            if($esProfesional==true){
  ...
  <main class="hoc container clear"> 
    ...
    <div class="content"> 
      <!-- ################################################################################################ -->
        <?php if(isset($_COOKIE["rol"])){ ?>
      <?php if($rol == $usuario){?>
      <h1>Usted no tiene autorización de estar en esta pagina</h1>
      <?php }else if($rol == $profesional && $_GET['id'] == $idUsuario){ ?>
           ...
           <center>
              <table>
                  ...
                  <tr>
                      ...
                      <td WIDTH="20%">
                          <a href="eliminarDisponibilidad.php?idProf=<?php echo $idUsuario ?>&id=<?php echo $fila['C_ID_DISPONIBILIDAD'] ?>"><img src="images/delete.png" style="max-width:15%;width:auto;height:auto;" title="Eliminar" alt="Eliminar"></a>
                      </td>
                  </tr>
                  <?php } ?>
              </table>
      ...
      <?php }else{
          echo "<h1>Error al cargar la página, si fue redireccionado aquí por un botón favor ponerse en contacto con soporte.</h1>";
      }?>
      <?php }else{?>
