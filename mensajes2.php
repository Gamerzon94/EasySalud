<?php
	...
	$usuario1 = "2";
        $profesional = "3";
        $farmacia = "4";
        $auxMensaje = obtenerMensaje($idMensaje);
?>
<!DOCTYPE html>
<!--
...
<div class="wrapper row3">
  <main class="hoc container clear"> 
    ...
    <div class="content"> 
      <!-- ################################################################################################ -->
        <?php if(isset($_COOKIE["rol"])){ 
            while($mensaje = mysqli_fetch_array($auxMensaje)){?>
      <h1><?php echo $mensaje["D_TITULO_MENSAJE"];?></h1> <br>
      De: <?php $auxDe = obtenerDatosPersonas($mensaje["C_ID_USUARIO_ENVIA"]);
      while($de = mysqli_fetch_array($auxDe)){
                      echo $de["D_NOMBRE_USUARIO"]." ".$de["D_APELLIDO_USUARIO"];
                      break;
                        }?><br>
      Para: <?php $auxPara = obtenerDatosPersonas($mensaje["C_ID_USUARIO_RECIBE"]);
      while($para = mysqli_fetch_array($auxPara)){
                      echo $para["D_NOMBRE_USUARIO"]." ".$para["D_APELLIDO_USUARIO"];
                      break;
                        }?><br>
      Fecha: <?php echo $mensaje["F_FECHA_REGISTRO"];?> hrs.<br>
      <table>
          <tr>
              <td>
                  <?php echo $mensaje["D_MENSAJE"];?>
              </td>
          </tr>
      </table>
      <input type="button" onclick=" location.href='mensajes3.php?id=<?php echo $mensaje["C_ID_USUARIO_RECIBE"]; ?>&titulo=<?php echo $mensaje["D_TITULO_MENSAJE"]; ?>' " value="Responder" name="boton" /> 
            <?php }}else{?>
