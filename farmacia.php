<?php
	...
	$rol = $_COOKIE["rol"];
        $idUsu = $_COOKIE["idUsu"];
        $datosFarmacia = cargarFarmacias($idUsu);
	}
	$admin = "1";
	$usuario = "2";
        $profesional = "3";
  ...
  <main class="hoc container clear"> 
    ...
    <div class="content">
       <?php if(isset($_COOKIE["rol"])){
        ...
        <button onclick="window.location.href='nuevaFarmacia.php'">Nueva Farmacia</button>
        <center>
            <table>
                <tr>
                    <td>
                ...
                <?php $count = 0;
                while($auxFarma = mysqli_fetch_array($datosFarmacia)){
                    $count++; 
                    $countSucur = 0;
                    $surcursales = contarSucursales($auxFarma['C_ID_FARMACIA']);?>
                <tr>
                    <td>
                        <?php echo $auxFarma["D_NOMBRE_FARMACIA"]; ?>
                    </td>
                    <td>
                        <?php echo $auxFarma["D_WEB_FARMACIA"]; ?>
                    </td>
                    <td>
                        <?php while ($auxSucur = mysqli_fetch_array($surcursales)){
                            $countSucur++;
                            echo "<center><a href=\"sucursalesFarmacia.php?id=".$auxFarma['C_ID_FARMACIA']."\">".$auxSucur["sucursales"]."</a></center>";
                        }
                        if($countSucur == 0){
                        echo "<center><a href=\"sucursalesFarmacia.php?id=".$auxFarma['C_ID_FARMACIA']."\">0</a></center>"; } ?>
                    </td>
                    <td WIDTH="20%">
                        <a href="modificarFarmacia.php?id=<?php echo $auxFarma['C_ID_FARMACIA'] ?>"><img src="images/edit.png" style="max-width:15%;width:auto;height:auto;" title="Modificar" alt="Modificar"></a>
                    </td>
                </tr>
                <?php } 
                if($count == 0){
                    echo "<tr><td colspan = '4'><center>Sin farmacias registradas.</center></td></tr>";
                }?>
            </table>
        </center>
           <?php } }else{ ?>
